...
class CommandBridgeNode(Node if ROS2_AVAILABLE else object):
    ...
    def translate_command(self, action: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        elif norm_action in ("ARM",):
            return {
                "action": "ARM",
                "arming": True,
                "topics": {"/mavros/cmd/arming": True}
            }
        elif norm_action in ("DISARM",):
            return {
                "action": "DISARM",
                "arming": False,
                "topics": {"/mavros/cmd/arming": False}
            }
        elif norm_action in ("TAKEOFF",):
            alt = float(params.get("altitude", 2.5))
            return {
                "action": "TAKEOFF",
                "altitude": alt,
                "flight_mode": "GUIDED",
                "topics": {"/takeoff": True, "/mavros/set_mode": "GUIDED"}
            }
        elif norm_action in ("LAND",):
            return {
                "action": "LAND",
                "flight_mode": "LAND",
                "topics": {"/land": True, "/mavros/set_mode": "LAND"}
            }
        elif norm_action in ("RTL", "RETURN", "RETURN_HOME", "SAFE_ACTION"):
            return {
                "action": "RTL",
                "flight_mode": "RTL",
                "topics": {"/rtl": True, "/mavros/set_mode": "RTL"}
            }
        elif norm_action in ("PAUSE_AUTONOMY", "PAUSE"):
            return {
                "action": "PAUSE_AUTONOMY",
                "flight_mode": "LOITER",
                "topics": {"/mavros/set_mode": "LOITER"}
            }
        elif norm_action in ("RESUME_AUTONOMY", "RESUME"):
            return {
                "action": "RESUME_AUTONOMY",
                "flight_mode": "GUIDED",
                "topics": {"/mavros/set_mode": "GUIDED"}
            }
        elif norm_action in ("RESET_MISSION", "RESET"):
            return {
                "action": "RESET_MISSION",
                "reset": True,
