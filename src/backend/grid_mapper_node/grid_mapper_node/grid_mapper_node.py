...
What this node does
-------------------
1. Subscribes to /drone_pose, /map, /confirmed_survivors (from fusion_node)
2. Converts each confirmed survivor's SLAM position → arena grid box ID
   using CoordinateTransformer (e.g. (6.3m, 4.7m) → "F4")
3. Maintains survivor grid location records decorated with arena cells
...
# /confirmed_survivors — RELIABLE so we never miss a confirmed detection
...
class GridMapperNode(Node):
    """
    ROS 2 Node that translates metric survivor positions to 1m grid cells.
    ...
    _survivors_cb    → process /confirmed_survivors, update registry
    ...
    def __init__(self) -> None:
        ...
        # /confirmed_survivors — multimodal confirmed survivors from fusion_node (authoritative source)
        ...
        self.get_logger().info(
            ...
            "/confirmed_survivors; publishing /survivor_grid_locations, /grid_map_overlay"
        )
    ...
    def _survivors_callback(self, msg: SurvivorArray) -> None:
        """
        Receive a batch of survivor detections from /confirmed_survivors.
