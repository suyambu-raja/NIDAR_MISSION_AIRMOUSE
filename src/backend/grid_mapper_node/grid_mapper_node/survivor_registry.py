...
class SurvivorRegistry:
    ...
    ------------------------------
        ...
        # On each /confirmed_survivors callback:
        for det in confirmed_survivors.detections:
            if det.is_confirmed:
    ...
    def add_or_update(
        ...
        # Preserve authoritative ID from fusion_node if provided (tracker_id >= 0)
        # ------------------------------------------------------------------
        assigned_id = tracker_id if tracker_id >= 0 else self._next_id
        record = SurvivorRecord(
            survivor_id=assigned_id,
            grid_box=grid_box,
            world_x=world_x,
            world_y=world_y,
